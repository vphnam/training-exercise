...
import { IPo, IPoShow } from '../services/interface/interface.service';
...
export class SearchfilterPipe implements PipeTransform {
  ...
  transform(PoList: any[], orderNoSearch: string, supplierNameSearch: string, stockSiteSearch: string, 
    stockNameSearch: string, orderDateSearch: Date, sentMailSearch: boolean): IPoShow[] {
    ...
    {
      PoList = PoList.filter((value: any) => 
      value.supplierName.toLocaleLowerCase().includes(supplierNameSearch.toLocaleLowerCase()));
    }
    if(stockSiteSearch != '' && stockSiteSearch != undefined)
    {
      PoList = PoList.filter((value: any) => 
      value.stockSite1.toLocaleLowerCase().includes(stockSiteSearch.toLocaleLowerCase()));
    }
